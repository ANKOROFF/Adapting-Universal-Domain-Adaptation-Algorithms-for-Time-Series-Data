...
class Algorithm(torch.nn.Module):
    ...
    def eval(self, src_pred_labels, src_true_labels, trg_pred_labels, trg_true_labels):
        """
        ...
        # Print metrics
        print(f"Source acc: {src_acc:.4f}, Target acc: {trg_acc:.4f}")
...
class UAN(Algorithm):
    ...
    def update(self, src_x, src_y, trg_x):
        ...
        src_disc = self.discriminator(src_feat)
        trg_disc = self.discriminator(trg_feat)
        ...
        src_domain_loss = F.binary_cross_entropy_with_logits(
            src_disc, 
            torch.ones(src_disc.size()).to(self.device)
        )
        trg_domain_loss = F.binary_cross_entropy_with_logits(
            trg_disc,
            torch.zeros(trg_disc.size()).to(self.device)
        )
        domain_loss = src_domain_loss + trg_domain_loss
        
        # Total loss
        loss = src_cls_loss + self.hparams["domain_loss_wt"] * domain_loss
...
class UniOT(Algorithm):
    ...
    def encode_features(self, x):
        ...
        print(f"Признаки после нормализации: {features.shape}")
    ...
    def update(self, src_x, src_y, trg_x):
        ...
        M = torch.cdist(src_feat, trg_feat)
        
        # Sinkhorn distance
        P = self.sinkhorn(src_feat, trg_feat, M)
        ot_loss = torch.sum(P * M)
        
        # Total loss
        loss = src_cls_loss + self.hparams["domain_loss_wt"] * ot_loss
    ...
    def sinkhorn(self, x, y, M, eps=0.1, max_iter=100):
        """
        Sinkhorn algorithm for Optimal Transport
        """
        b = torch.ones(y.size(0)).to(self.device) / y.size(0)
        a = torch.ones(x.size(0)).to(self.device) / x.size(0)
