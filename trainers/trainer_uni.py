...
class cross_domain_trainer(object):
   ...
   This class contain the main training functions for our AdAtime
            ...
            for run_id in range(self.num_runs):  # specify number of consecutive runs
                ...
                for epoch in range(1, self.hparams["num_epochs"] + 1):
                    ...
                    if self.da_method=='DANCE':
                        ...
                        acc, f1, H = self.evaluate_tfac(
                            self.algorithm.encoder,
    ...
    def evaluate_tfac(self, encoder, classifier, trg_test_dl, src_test_dl):
        feature_extractor = encoder.to(self.device)
        ...
        with torch.no_grad():
            for batch in src_test_dl:
                ...
                source_labels.extend(y.cpu().numpy())
        
        # Сохраняем предсказания и истинные метки
        self.trg_pred_labels = np.array(target_preds)
        self.trg_true_labels = np.array(target_labels)
        
        # Вычисление метрик
        target_acc = accuracy_score(target_labels, target_preds)
        target_f1 = f1_score(target_labels, target_preds, average='weighted')
        source_acc = accuracy_score(source_labels, source_preds)
        source_f1 = f1_score(source_labels, source_preds, average='weighted')
        
        # Вычисление H-score
        h_score = 2 * (target_acc * source_acc) / (target_acc + source_acc)
